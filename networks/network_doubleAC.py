...
class DoubleACNetwork(BaseNetwork):
  def __init__(self, scope, config, action_size, total_steps_tensor=None):
    super(DoubleACNetwork, self).__init__(scope, config, action_size, total_steps_tensor)
    self.summaries_term = []
  ...
  def build_policy_over_options_net(self):
    with tf.variable_scope("policy_over_options"):
      out = tf.stop_gradient(self.fi_relu)
      self.pi_over_options = layers.fully_connected(out, num_outputs=self.nb_options,  # + self.action_size,
                                                    activation_fn=tf.nn.softmax,
                                                    biases_initializer=None,
                                                    variables_collections=tf.get_collection("variables"),
                                                    outputs_collections="activations", scope="pi_over_options")
      self.summaries_option.append(tf.contrib.layers.summarize_activation(self.pi_over_options))
  ...
  def build_option_q_val_net(self):
    with tf.variable_scope("option_q"):
      ...
      self.q_val = layers.fully_connected(out, num_outputs=1,
                                          activation_fn=None,
                                          variables_collections=tf.get_collection("variables"),
      ...
      self.all_options_directions_placeholder = tf.placeholder(shape=[None, self.nb_options, self.sf_layers[-1]], dtype=tf.float32,
                                                         name="all_options_directions")
      out = tf.add(tf.tile(tf.expand_dims(tf.stop_gradient(self.fi), 1), [1, self.nb_options, 1]), self.all_options_directions_placeholder)
      self.q_val_for_v = tf.map_fn(lambda x: layers.fully_connected(x, num_outputs=1,
                                          activation_fn=None, reuse=True,
                                          variables_collections=tf.get_collection("variables"),
                                          outputs_collections="activations", scope="q_val"), out)
      self.q_val_for_v = tf.squeeze(self.q_val_for_v, 2)
      # self.possible_option_directions = tf.placeholder(shape=[None, self.sf_layers[-1]],
      #                                 dtype=tf.float32, name="possible_option_directions")
      self.v = tf.reduce_sum(self.pi_over_options * self.q_val_for_v, 1)
      self.summaries_critic.append(tf.contrib.layers.summarize_activation(self.v))

      return out

  def get_responsible_options(self, pi_over_options, o):
    indices = tf.stack([tf.range(tf.shape(o)[0]), o], axis=1)
    responsible_options = tf.gather_nd(pi_over_options, indices)
  ...
  def build_losses(self):
    ...
    self.responsible_options = self.get_responsible_options(self.pi_over_options, self.options_placeholder)
    ...
    with tf.name_scope('critic_loss'):
      td_error = self.target_return - self.q_val
    self.critic_loss = tf.reduce_mean(0.5 * self.config.critic_coef * tf.square(td_error))

    with tf.name_scope('termination_loss'):
      self.term_loss = tf.reduce_mean(
        self.termination * (tf.stop_gradient(self.q_val) - tf.stop_gradient(self.v) + 0.01))

    with tf.name_scope('entropy_loss'):
      self.entropy_loss = -self.entropy_coef * tf.reduce_mean(self.option * tf.log(self.option + 1e-7))

    with tf.name_scope('entropy_pi_over_options_loss'):
      self.entropy_pi_over_options_loss = -self.entropy_coef * tf.reduce_mean(
        self.pi_over_options * tf.log(self.pi_over_options + 1e-7))

    with tf.name_scope('policy_loss'):
      self.policy_loss = -tf.reduce_mean(tf.log(self.responsible_actions + 1e-7) * tf.stop_gradient(
        eigen_td_error))

    with tf.name_scope('pi_over_options_loss'):
      self.pi_over_options_loss = -tf.reduce_mean(tf.log(self.responsible_options + 1e-7) * tf.stop_gradient(
        td_error))

    self.option_loss = self.policy_loss + self.pi_over_options_loss - self.entropy_loss - self.entropy_pi_over_options_loss
  ...
  def build_network(self):
    with tf.variable_scope(self.scope):
      ...
      _ = self.build_policy_over_options_net()
