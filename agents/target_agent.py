...
class TargetAgent(BaseAgent):
  ...
  def init_play(self, sess, saver):
    ...
    # self.sf_episode_buffer = deque()
    self.reward_pred_episode_buffer = deque()
    # self.ms_aux = self.ms_sf = \
    self.ms_option = self.ms_reward = self.ms_reward_i = None
  ...
  def SF_prediction(self):
    if len(self.sf_episode_buffer) > self.config.observation_steps and \
      ...
      while not coord.should_stop():
        ...
        while not self.done:
          ...
          if self.total_steps > self.config.observation_steps:
            ...
            self.option_prediction(s, self.old_option, s1, self.option, self.action, self.primitive_action)
