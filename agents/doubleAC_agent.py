...
class DoubleACAgent(EigenOCAgentDyn):
  def __init__(self, game, thread_id, global_step, config, global_network, barrier):
    super(DoubleACAgent, self).__init__(game, thread_id, global_step, config, global_network, barrier)
  ...
  def option_evaluation(self, s):
    ...
    self.pi_over_options = self.sess.run(self.local_network.pi_over_options, feed_dict=feed_dict)[0]
    self.option = np.random.choice(self.pi_over_options, p=self.pi_over_options)
    self.option = np.argmax(self.pi_over_options == self.option)
  ...
  def policy_evaluation(self, s):
    if self.total_steps > self.config.eigen_exploration_steps:
      feed_dict = {self.local_network.observation: [s],
                   ...
                   self.local_network.all_options_directions_placeholder: [self.global_network.directions]
                   }
      tensor_list = [self.local_network.sf, self.local_network.q_val, self.local_network.v,
                     self.local_network.termination, self.local_network.eigen_q_val, self.local_network.option]

      sf, q_value, value, o_term, eigen_q_value, option = self.sess.run(tensor_list, feed_dict=feed_dict)
  ...
  def option_prediction(self, s, s1, r):
    ...
    if self.option_counter == self.config.max_update_freq or self.done or (
      ...
      if self.done:
        ...
        value, q_value, q_eigen = self.sess.run(
          [self.local_network.v, self.local_network.q_val, self.local_network.eigen_q_val], feed_dict=feed_dict)
        q_value = q_value[0][0]
        value = value[0]
  ...
  def train_option(self, bootstrap_value, bootstrap_value_mix):
    ...
    feed_dict = {self.local_network.target_return: discounted_returns,
                 ...
                 self.local_network.option_direction_placeholder: self.global_network.directions[
                   np.array(options, dtype=np.int32)],
                 self.local_network.target_eigen_return: discounted_eigen_returns,
                 self.local_network.all_options_directions_placeholder: np.repeat([self.global_network.directions], len(observations), 0)
                 }
